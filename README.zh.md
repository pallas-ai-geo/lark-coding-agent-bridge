...
| `/autotopic on\|off\|status` | 在话题群中，仅让新话题首帖自动触发 |
...
私聊不需要 @。群和话题群默认必须 `@bot`；`@all` 会被忽略。话题群里管理员可以发 `/autotopic on`，让新话题首帖自动触发，但话题下面的后续回复仍然需要 `@bot`。支持的云文档评论里 @bot 就会触发回复。
...
| **话题首帖自动回复** | 哪些话题群的新话题首帖可以不 @bot 触发 | `/autotopic on`（当前话题群） | `/autotopic off`（当前话题群） |
...
- **只自动回复新话题首帖** → 在话题群里发 `/autotopic on`；话题内后续回复仍需 `@bot`
...
- `/autotopic on` 比全局关闭“群里需要 @bot”更窄：它只放行当前话题群的新话题首帖。
...
`allowedUsers` / `admins` 填用户 `open_id`，`allowedChats` 和 `autoReplyTopicChats` 填群 `chat_id`。手动找 ID 最简单的办法：让对方给 bot 发条消息（群里就 @ 它一下），然后看当前 profile 的日志：
