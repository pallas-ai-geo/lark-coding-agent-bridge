...
describe('topic message quote handling', () => {
  ...
  it('falls back to the topic root when a topic-group event has no recoverable threadId', async () => {
    // Backfill lookup returns nothing. We still have root_id/message_id on
    // topic-root events, so keep scope/reply routing anchored to that thread
    // shape instead of falling back to the whole chat.
  ...
  it('fetches regular group reply history from the thread container', async () => {
      ...
      threadMessages: [
        {
          ...
          thread_id: 'om_quote_target',
          root_id: 'om_quote_target',
          msg_type: 'text',
          body: { content: JSON.stringify({ text: '普通群回复串里的历史' }) },
    ...
    const prompt = h.agent.runOptions[0]?.prompt ?? '';
    expect(prompt).toContain('<thread_history>');
    expect(prompt).toContain('普通群回复串里的历史');
    expect(h.channel.rawClient.im.v1.message.list).toHaveBeenCalledWith({
      params: expect.objectContaining({
        container_id_type: 'thread',
